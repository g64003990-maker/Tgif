...
function showLose() {
	...
	$('#messagebar h3').text('Your willpower has reached zero!');
	$('#popupClose').hide();
...
function showLoseLetter() {
	$('#popup').fadeOut();
	$('#messagebar').html('<h1></h1><h3></h3><div class="ichiranBTN ichiranSub"id="popupClose">Ok</div></div>');
	/*var html = '<div class="intropic"><div class="introcrop"><div id="letterAvatar"><img id="hairstyleBack" src="resources/img/mc/hair/3/back/2.png"><img id="shoes" src="resources/img/mc/bot/4/shoes/3.png"><img id="bot" src="resources/img/mc/bot/4/3.png"><img id="top" src="resources/img/mc/top/4/4.png"><img id="nails" src="resources/img/mc/top/4/nails/2.png"><img id="head" src="resources/img/mc/head.png"><img id="makeup" src="resources/img/mc/makeup/2/makeup.png"><img id="face" src="resources/img/mc/face/4.png"><img id="makeupLips" src="resources/img/mc/makeup/2/lips/2.png"><img id="lips" src="resources/img/mc/face/lips/2.png"><img id="hairstyle" src="resources/img/mc/hair/3/2.png"><img id="jewelry" src="resources/img/mc/top/4/jewelry/2.png"><img id="sumaho" src="resources/img/mc/sumaho/5.png"></div></div></div><p>Attention all managers,</p><p>Unfortunately, we regret to inform you that our newest intern in the secretary program was unable to successfully complete the course.</p><p>While her body took readily to all improvements (see attached image), she lacked the willpower to succeed in administrative assistant tasks.</p><p>That said, she excelled at interpersonal relationships and will join the employee “morale” program.</p><p>While she is eager to get started, we are having difficulty choosing a name. “Barbie” and “Candie” are the leading options, but we will be holding a contest! Please reply to this message with the name you’d like to give her! The winning will be chosen by HR, and you will have first dibs on our newly named employee!</p><p>Management</p><p style="font-weight:bold;">Game over: You lose</p>'*/
...
function belowpile() {
	...
	var html = '<h1>Below Pile</h1><h2>Choose one card from your hand to return to the effort deck (on the bottom).</h2><div id="ichiran" class="selectable">';
		...
		hand.forEach(function(item) {
		html += '<div class="card selectable" style="background:url(resources/img/cards/' + item.img + '.jpg) no-repeat center center;"><div class="titleBar"><div class="cardEffort"><span class="cardEffortVal">' + item.effort + '</span>Effort</div><h3>' + item.title + '</h3>';
		if (item.willpower == 1) {
		html += '<div class="cardBurn"><div></div></div>'; }
		else {
		html += '<div class="cardBurn"><div></div><div></div></div>'; }
...
function confirmBelowPile(specCard) {
	...
	$('#ichiran.selectable .card.selectable').each(function(x) { 
		if ( $(this).hasClass('selected') ) {
			...
			$('#messagebar h3').text( '"' + $(this).find('h3').text() + '" moved to Effort Deck');
...
function copy() {
	...
	var html = '<h1>Copy</h1><h2>Choose one card from your hand with a special ability, then copy that ability to this card.</h2><div id="ichiran" class="selectable">';
		...
		if (item.specialKey !== '' && item.specialKey !== 'copy' ) {
			html += '<div class="card selectable" data-place="' + handplace + '" style="background:url(resources/img/cards/' + item.img + '.jpg) no-repeat center center;"><div class="titleBar"><div class="cardEffort"><span class="cardEffortVal">' + item.effort + '</span>Effort</div><h3>' + item.title + '</h3>';
...
function destroy() {
	...
	hand.forEach(function(item) {
		html += '<div class="card destroyable" data-burn="1" style="background:url(resources/img/cards/' + item.img + '.jpg) no-repeat center center;"><div class="titleBar"><div class="cardEffort"><span class="cardEffortVal">' + item.effort + '</span>Effort</div><h3>' + item.title + '</h3>';
		if (item.willpower == 1) {
		html += '<div class="cardBurn"><div></div></div>'; }
		else {
		html += '<div class="cardBurn"><div></div><div></div></div>'; }
...
function confirmDestroy(specCard) {
	...
	$('#ichiran.destroyable .card.destroyable').each(function(x) { 
		if ( $(this).hasClass('selected') ) {
			...
			$('#messagebar h3').text( '"' + $(this).find('h3').text() + '" was removed from the game.');
			hand.splice((x),1)
		}
	});
	refreshGame()
...
function confirmDouble(specCard) {
	...
	$('#ichiran.selectable .card.selectable').each(function(x) { 
		if ( $(this).hasClass('selected') ) {
			...
			$('#messagebar h3').text( '"' + $(this).find('h3').text() + '" was doubled!');
